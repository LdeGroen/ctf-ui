...
export const AppShell = ({ title, logoSrc, children }) => (
    <div className="min-h-screen w-full font-sans" style={{ backgroundColor: CTF_GREEN }}>
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <header className="absolute top-4 right-4 h-16 sm:h-20 z-20">
            <img
                src={logoSrc || `${(typeof process !== 'undefined' && process.env && process.env.PUBLIC_URL) || ''}/logo.png`}
                alt="Café Theater Festival Logo"
...
export const StatusMessage = ({ error = false, children }) => (
    <p className={`text-center ${error ? 'text-red-200' : 'text-white/80'}`}>{children}</p>
);

// -------------------------------------------------------------- SelectorNav
// De jaar/stad-pillen-navigatie (identiek in makers en cafes).
// items: array van strings; value/onChange: geselecteerde waarde.
export const SelectorNav = ({ items, value, onChange, accent = CTF_BLUE }) => (
    <nav className="flex justify-center flex-wrap gap-3 sm:gap-4 mb-10">
        {items.map((item) => (
            <button
                key={item}
                onClick={() => onChange(item)}
                className={`px-6 py-3 text-lg font-semibold rounded-lg shadow-md transition-transform transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#20747F] focus:ring-white ${
                    value === item ? 'bg-white text-[#20747F]' : 'text-white'
                }`}
                style={{ backgroundColor: value !== item ? accent : '' }}
            >
                {item}
            </button>
        ))}
        ...
        >
            <div className="aspect-w-4 aspect-h-3">
                <img
                    ...
                    onError={(e) => { e.target.onerror = null; e.target.src = placeholder(errorText); }}
                />
            </div>
            <div className="p-4" style={{ backgroundColor: accent }}>
                <h3 className="text-lg font-bold text-white truncate text-center">{title}</h3>
            </div>
        </a>
    );
};
...
export const SubmitButton = ({ loading, label, loadingLabel, className = '', ...rest }) => (
    <button type="submit" disabled={loading} className={className} {...rest}>
...
export const FormField = ({ label, name, variant = 'light', textarea = false, rows = 4, ...rest }) => (
    <div>
        {label && <label htmlFor={name} className={LABEL_STYLES[variant]}>{label}</label>}
        {textarea
            ? <textarea id={name} name={name} rows={rows} className={FIELD_STYLES[variant]} {...rest} />
            : <input id={name} name={name} className={FIELD_STYLES[variant]} {...rest} />}
    </div>
);
...
export const useSubmitForm = (initial, submitFn) => {
    ...
    const reset = () => { setFormData(initial); setStatus('idle'); };

    return { formData, setFormData, handleChange, handleSubmit, status, setStatus, reset };
};
