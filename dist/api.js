...
const API_URL = typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_API_URL || typeof process !== 'undefined' && process.env && process.env.REACT_APP_API_URL || FALLBACK_API_URL;
...
export class Databases {
  async listDocuments(_db, collectionIdOrName, queries = []) {
    ...
    if (equalIds && equalIds.length > 0) {
      const results = await Promise.all(equalIds.map(id => apiFetch(`/api/public/${resource}/${id}`).catch(() => null)));
    ...
    const params = new URLSearchParams({
      ...
      page: String(page),
      ...Object.fromEntries(Object.entries(equalFilters).map(([k, v]) => [k, String(v)]))
    });
...
// ---------------------------------------------------------------------------
// Directe fetch-helper voor de display-apps (wieiswie/makers/cafes): GET naar
// de publieke API met een 15s-timeout; geeft json.data terug.
export { API_URL };
